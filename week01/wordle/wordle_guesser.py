...
def eliminate_nonmatching(candidates: list, guess: str, feedback: list) -> list:
    """Returns a list of new candidates that only contains those words from <candidates> that
       are consistent with <feedback> list (see evaluate_guess) for <guess>."""
    new_candidates = list()
    for candidate in candidates:  # check for all remaining candidates
        eliminate = False  # can we eliminate the current word?
        for idx in range(5):  # check for all positions
            # ignore a word if a position does not match the guess for a "positional match" feedback (X)
            if feedback[idx] == 2 and guess[idx] != candidate[idx]:
                    eliminate = True
            # ignore a word if a letter a "non-positional match" is not in the candidate (O)
            # TODO: we also know that the letter is NOT at current position, check that as well!
            if feedback[idx] == 1 and guess[idx] not in candidate:
                    eliminate = True
            # ignore a word if a letter a "mismatch" letter is in the candidate (-)
            if feedback[idx] == 0 and guess[idx] in candidate:
                    eliminate = True
            if eliminate:  # if we already know that a wpord can be ignored, there's no need to check remaining positions
                break  # leave current loop immediately
        if not eliminate:  # if word has not been eliminated: add it to the new candidate list
            new_candidates.append(candidate)
    return new_candidates

def guesser_game(word:str=None, silent:bool=False)->int:
    if not silent: print("Welcome to a new game.\n\n")
    if not word:  # let user choose a word
        while True:
            word = input("Which word to guess: ").upper()
            if len(word) != 5 or not word in words:
                print("Please pick an existing five letter word (from our list or 1367 words)!")
            else:
                break
    candidates = words
    guess = ''
    ...
    while not guess==word:
        ...
        if not silent: print("I guess: {}. Feedback is: {}".format(guess, " ".join([['-', 'O','X'][x] for x in feedback])))
        candidates = eliminate_nonmatching(candidates, guess, feedback)
        if not silent: print("Candidates left: {}".format(len(candidates)))
    if not silent: print("Congratulations, I got it in {} guesses. :-)".format(guess_count))
    return guess_count


def auto_guesser(word:str=None, iterations:int=1000)->float:
    """Make 1000 runs for the given word and return the average number of guesses."""
    guesses = 0
    for i in range(iterations):
        guesses += guesser_game(word, silent=True)
    print("Average for {} is {}".format(word, guesses/iterations))
    return guesses/1000

if __name__=='__main__':
    # let player pick a word and print details
    guesser_game()

    # uncomment for automatic runs
    # for w in ['HOUSE', 'PLATE', 'WORLD', 'GREAT', 'GREET', 'ADIEU']:
    #     auto_guesser(w, 10000)